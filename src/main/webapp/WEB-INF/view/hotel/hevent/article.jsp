...
<div class="roberto-news-area section-padding-100-0">
	<div class="container">
		<div class="col-12 lg-9 mb-40" style="font-size: 16px;">
			...
			<div align="center">
				<button class="btn btn-gray" onclick="location.href='<%=cp%>/hotel/hevent/list?${query}';">목록으로</button>
			</div>
...
<script>
	...
	function replyList(page){
		var url = "<%=cp%>/hotel/hevent/replyList";
		$.ajax({
			type:"get",
			url:url,
			data:{
	...
	$(".btn_addEvent").click(function(){
		...
		var url = "<%=cp%>/hotel/hevent/insertReply";
		$.ajax({
			type:"post",
			url:url,
			data:{
	...
	$("body").on("click",".deleteReplyBtn", function(){
		...
		var url = "<%=cp%>/hotel/hevent/deleteReply";
		$.ajax({
			type:"post",
			url:url,
			data:{
