...
<head>
	...
	.layer .content{display:inline-block;width: 510px; height:500px;padding: 40px;background:#fff;border-radius: 40px;vertical-align:middle; border:1px solid efefef; margin-top:50px;}
...
<script type="text/javascript">
 function checkAll() {
		for(i=0; i < stepForm.checkbox.length; i++) {
			stepForm.checkbox[i].checked = true;
		}
	}
 ...
 function gostep2(){
	var cnt = 0;
		...
		if(cnt!=2){
			alert("이용 약관에 동의하셔야 서비스 이용 가능합니다.");
			return;
		}
  ...
  <div class="content">
  	<h1>회원가입</h1>
  		<form name="stepForm" method="POST">
  			<table>
					<tr>
						<td style="text-align:left;"><h3><b class="t_blue">&nbsp;*</b>&nbsp;이용 약관</h3></td>
					</tr>
					<tr>
						<td>
							<textarea rows="10" cols="30" style="width:500px; height:150px; margin-bottom:5px;" readonly="readonly">
제1장 총칙

제1조 (목적)
이 약관은 와유㈜ (영문명 WAYOU co,.ltd, 이하 "회사"라 합니다)이 인터넷 서비스(이하 "서비스"라 합니다)를 제공함에 있어 회사와 회원간의 권리, 의무, 이용조건 및 절차, 기타 필요한 사항을 규정함을 목적으로 합니다.
...
제3조 (약관외 준칙)
이 약관에 명시되지 아니한 사항에 대해서는 전기통신기본법, 전기통신사업법, 정보통신망 이용촉진등에 관한 법률, 국외여행표준약관 및 기타 관련 법령의 규정에 따릅니다.
...
② 회원가입신청에 대한 회사의 승낙시점은 회원가입이 되었다는 내용의 웹브라우저가 열린 때, 또는 시스템상으로 로그인이 가능해진 시점으로 봅니다.

제7조(회원가입 신청)
① 회원가입 신청자는 서비스를 통하여 본 약관에 동의한다는 의사표시와 함께 회사가 요청하는 소정의 가입양식에 회원정보를 기재하여 신청합니다.
② 회원가입 신청양식에 기재하는 모든 회원정보는 실제 데이터인 것으로 간주 하며실명이나 실제 정보를 입력하지 않은 사용자는 법적인 보호를 받을 수 없으며, 서비스사용의 제한을 받으실 수 있습니다.
...
제9조(회원탈퇴)
① 회원은 회원가입계약을 해지하고자 하는 경우에는 서비스내 "훼밀리회원탈퇴" 메뉴에서 탈퇴신청을 하여야 하며 회원의 탈퇴신청에 대해 회사는 빠른 시간내로 탈퇴처리를 할 의무가 있습니다.
...
③ 회원은 이용 신청 시 기재/입력한 사항이 변경되었을 경우에는 즉시 수정을 하여야 합니다.

제12조(서비스의 구매)
회원은 아래와 같은 절차에 따라 롯데관광 홈페이지를 통하여 여행상품 또는 서비스 및 제휴 서비스를 포인트로 구매할 수 있습니다.
① 성명, 주소, 전화번호 입력
② 재화 또는 용역의 선택
③ 결제 방법의 선택
④ 기타 "회사"가 별도로 정하는 절차
...
제13조(서비스의 요금)
회사가 제공하는 서비스는 무료로 합니다. 다만 회사는 추후에 서비스를 유료로 할 수 있으며, 그 구체적인 내용에 대하여는 회원과 별도로 합의합니다.

제14조 (서비스의 개시)
서비스는 회사가 제6조에 따라서 이용신청을 승낙한 때로부터 즉시 개시됩니다. 
다만 제16조에 해당하는 경우 서비스의 개시시점이 연기될 수 있습니다.
...
② 회사는 제공하는 서비스중 일부에 대한 서비스 이용시간을 별도로 정할 수 있으며, 이 경우 그 이용시간을 사전에 회원에게 공지 또는 통지합니다.
...
제17조 (서비스 제공의 중지)
회사는 다음 각호의 1에 해당하는 경우 서비스의 제공을 중지할 수 있습니다.
...
제19조 (개인정보의 보호)
① 회사는 서비스와 관련하여 기득한 회원의 개인정보를 본인의 사전 승낙없이 타인에게 누설, 공개 또는 배포할 수 없습니다. 다만, 다음 각 호의 1에 해당하는 경우에는 그러하지 아니합니다.
1. 관계법령에 의하여 수사상의 목적으로 관계기관으로부터 요구받은 경우
2. 정보통신윤리위원회의 요청이 있는 경우
3. 기타 관계법령에 의한 경우
4. 특정 개인을 식별할 수 없는 통계작성, 홍보자료, 학술연구 등의 목적일 때
② 제1항의 범위내에서, 회사는 광고업무와 관련하여 회원 전체 또는 일부의 개인정보에 관한 통계 자료를 작성하여 이를 사용할 수 있고, 서비스를 통하여 회원의 컴퓨터에 쿠키를 전송할 수 있습니다. 이 경우 회원은 쿠키의 수신을 거부하거나 쿠키의 수신에 대하여 경고하도록 사용하는 컴퓨터의 브라우저의 설정을 변경할 수 있습니다.
...
⑤ 회원은 ID 및 비밀번호를 도난당하거나 제3자에게 사용되고 있음을 인지한 경우에는 바로 회사에 통보하고 회사의 지시가 있는 경우에는 그에 따라야 합니다.

제22조(게시물 또는 내용물의 삭제)
회사는 서비스의 게시물 또는 내용물이 다음 각호의 1에 해당한다고 판단되는 경우 회원에게 사전통지나 동의없이 이를 삭제할 수 있습니다.
...
부칙
제1조 (시행일) 이 약관은 2013년 11월 1일부터 시행합니다.
제2조 (경과조치) 이 약관 시행전에 가입한 회원은 시행일부터 이 약관의 적용을 받습니다.
					...
					<tr>
						<td style="text-align:right; padding-bottom:15px">
							<label class="checkbox">이용 약관을 확인하였으며 동의합니다.(필수)
					...
					<tr>
						<td style="text-align:left;"><h3><b class="t_blue">&nbsp;*</b>&nbsp;개인정보수집에 대한 이용 안내</h3></td>
					</tr>
					<tr>
						<td>
							<textarea rows="10" cols="30" style="width:500px; height:150px; margin-bottom:5px; " readonly="readonly">
1. 개인정보 수집 및 이용목적
회사는 여행상품의 예약 및 여행관련 서비스 제공 등의 업무처리를 위하여 고객으로부터 최소한의 필수정보를 수집하며, 제공한 모든 개인정보는 별도의 동의가 없는 한 해당목적 이외의 다른 목적으로 사용하지 않습니다. 회사는 수집한 개인정보를 다음의 목적을 위하여 사용합니다.

가. 서비스 제공에 관한 계약 이행 및 서비스 제공에 따른 요금정산

여행상품 예약(일정/숙박/티켓 등), 예약내역의 확인 및 상담, 컨텐츠 제공, 여행서비스 이용 시 회원의 우대, 구매 및 요금결제, 물품배송 또는 청구서 등 발송, 본인인증 및 금융서비스 등
...
나. 회사는 고객의 인권을 침해할 우려가 있는 민감한 개인정보항목(인종, 종교, 사상, 정치적 성향, 건강상태 등)은 수집하지 않습니다

3. 동의를 거부할 권리 및 동의를 거부할 경우의 불이익
개인정보 주체는 개인정보 수집, 이용에 대한 동의를 거부할 권리가 있습니다. 동의를 거부할 경우 회원가입이 불가함을 알려드립니다.
							</textarea>
						</td>
					</tr>
					<tr>
						<td style="text-align:right; padding-bottom:15px">
							<label class="checkbox">개인정보수집에 대한 이용 안내를 확인하였으며 동의합니다.(필수)
								<input name="checkbox" type="checkbox" />
								<span class="lbl">동의함</span>
							</label>
						</td>
					...
					<tr>
						<td>
						<button type="button" onclick="checkAll()" class='button btn_blue'>전체 동의</button>
						<button type="button" onclick="gostep2()" class='button btn_blue'>다음 단계</button>
						</td>
					</tr>
			</table>
		</form>
