				...
				<table class=tb style="font-size: 15px;margin-top:20px;"> 
					...
					<c:forEach var="dto" items="${list}">
						<tr align="center" bgcolor="#ffffff" height="40"
							...
							<td>${dto.listNum}</td>
							<td style="cursor: pointer;"><a href="<%=cp%>/ticket/notice/article?noticeNum=${dto.noticeNum}">${dto.subject}</a></td>
							<td>${dto.adminName}</td>
							<td>${dto.created}</td>
							<td>${dto.hitCount}</td>
						</tr>
...
<script>
	function sendSearch(){
		if(!$("input[name=searchValue]").val()){
			location.href="<%=cp%>/ticket/notice/list";
			return false;
		}
		var searchKey = $("select[name=searchKey]").val();
		var searchValue=$("input[name=searchValue]").val();
		$("input[name=searchValue]").val("");
		location.href="<%=cp%>/ticket/notice/list?searchKey="+searchKey+"&searchValue="+encodeURIComponent(searchValue);
	}
</script>