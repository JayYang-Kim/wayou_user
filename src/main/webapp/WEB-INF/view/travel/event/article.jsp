...
<div class="roberto-news-area section-padding-100-0">
	<div class="container">
		<div class="col-12 lg-9 mb-40" style="font-size: 16px;">
			...
			<c:forEach var="dto" items="${event.files}">
			...
			<div align="center">
				<button class="btn btn-gray" onclick="location.href='<%=cp%>/travel/event/list?${query}';">목록으로</button>
			</div>
...
<script>
	...
	function replyList(page){
		var url = "<%=cp%>/travel/event/replyList";
		$.ajax({
			type:"get",
			url:url,
			data:{
	...
	$(".btn_addEvent").click(function(){
		...
		var url = "<%=cp%>/travel/event/insertReply";
		$.ajax({
			type:"post",
			url:url,
			data:{
	...
	$("body").on("click",".deleteReplyBtn", function(){
		replyCode = $(this).closest(".replyBox").attr("data-replyCode");
		userIdx = $(this).closest(".replyBox").attr("data-userIdx");
		if(!confirm("삭제하시겠습니까?")){
			return false;
		}
		var url = "<%=cp%>/travel/event/deleteReply";
		$.ajax({
			type:"post",
			url:url,
			data:{
